...
public class DandleGame
{
	...
	{
		var user = UserPoints.Where(x => x.UserId == userId).First();
	...
	public DandleGame SelectRandomWord()
	{
		...
		var targetWord = DandleDictionary.OrderBy(x => new Random().Next()).FirstOrDefault()
	...
	{
		var result = CurrentSuggestedWords
			.Where(x => x.DandleWord.Word == word && x.User.Id == user.Id)
			.FirstOrDefault();
	...
	{
		var result = CurrentSuggestedWords
			.Where(x => x.DandleWord.Word == word 
	...
	{
		return CurrentVotes.Select(x => new PollChoiceInfo(x.Word, 0)).ToList();
	}
	...
	public void ClearSuggestionVoteList()
	{
		CurrentVotes = new List<DandleVotes>();
	}
	...
	{
		return CurrentSuggestedWords.Count == 1;
	}
	...
	public bool OnlyOneSuggestionSubmitted()
	{
		return CurrentVotes.Count == 1;
	}
}