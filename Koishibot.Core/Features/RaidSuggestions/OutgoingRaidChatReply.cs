...
{
	public static async Task PostRaidSuggestionsOpen(this ITwitchIrcService BotIrc)
	{
		await BotIrc.BotSend("Raid suggestions are now open for the next 2 minutes, please use !suggest (user) to nominate a streamer!");
	}

	public static async Task RaidStatus(this ITwitchIrcService irc, Code code)
	{
	...
	public static async Task PostSuggestionResult(this ITwitchIrcService irc, Code code, (string user, string streamer) e)
	{
		var message = code switch
		{
	...
	public static async Task PostRaidCall(this ITwitchIrcService irc)
	{
		var message = $"SabaPing E-LEE-SHA GRIFFIN'S KOI KIN RAID‽ SabaPing";
		await irc.BotSend(message);
	}

	public static async Task RaidTarget(this ITwitchIrcService irc, Code code, string user, string? streamer = "")
	{
		var message = code switch
		{
